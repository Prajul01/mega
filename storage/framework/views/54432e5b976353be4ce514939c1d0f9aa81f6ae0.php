    ...
    <div class="dd" id="nestable">
        <ol class="dd-list has-header">
            <?php $__currentLoopData = $employee_types; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $employee_type): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <li class="dd-item dd3-item list-item" data-id="<?php echo e($employee_type->id); ?>">
                    <div class="dd-handle dd3-handle"></div>
                    <div class="custom-handle-flex">
                        <div class="icon-image-name">
                            <div>
                                <h6 class=" mb-0">
                                    <?php echo e($employee_type->title); ?> | <small
                                        class="<?php echo e($employee_type->status == 'active' ? 'badge badge-primary' : 'badge badge-danger'); ?>"><?php echo e($employee_type->status == 'active' ? 'Displayed' : 'Not Display'); ?></small>
                                </h6>
                            </div>
                        </div>
                        <div style="display: flex">
                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('job-edit')): ?>
                                <a href="<?php echo e(route('admin.employee_type.edit', base64_encode($employee_type->id))); ?>"
                            ...
                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('job-delete')): ?>
                                &nbsp;
                                <button type="button" onclick="COnfirmDelete('<?php echo e($employee_type->id); ?>')"
                                    data-id="<?php echo e($employee_type->id); ?>" class="btn btn-sm btn-outline-danger">
    ...
    <script>
        $(document).ready(function() {
            var updateOutput = function(e) {
                ...
                $.ajax({
                    method: "POST",
                    url: "<?php echo e(route('admin.employee_type.order')); ?>",
                    data: {
        ...
        function COnfirmDelete(elem) {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        method: "POST",
                        url: '<?php echo e(route('admin.employee_type.destroy')); ?>',
                        data: {
                    ...
                    swal("Cancelled", "Your employee_type is safe :)", "error");
                }
            });
        }
    </script>
<?php $__env->stopPush(); ?>
<?php /**PATH /home/megajobn/public_html/resources/views/admin/employeeType/components/list.blade.php ENDPATH**/ ?>