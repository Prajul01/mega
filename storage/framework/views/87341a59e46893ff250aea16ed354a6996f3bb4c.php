    ...
    <div class="dd" id="nestable">
        <ol class="dd-list has-header">
            <?php $__currentLoopData = $tenders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tender): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <li class="dd-item dd3-item list-item" data-id="<?php echo e($tender->id); ?>">
                    <div class="dd-handle dd3-handle"></div>
                    <div class="custom-handle-flex">
                        <div class="icon-image-name">
                            <div>
                                <h6 class=" mb-0">
                                    <?php echo e($tender->title); ?> | <small
                                        class="<?php echo e($tender->status == 'active' ? 'badge badge-primary' : 'badge badge-danger'); ?>"><?php echo e($tender->status == 'active' ? 'Displayed' : 'Not Display'); ?></small>
                                </h6>
                            </div>
                        </div>
                        <div style="display: flex">
                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('tender-edit')): ?>
                                <a href="<?php echo e(route('admin.tender.edit', base64_encode($tender->id))); ?>"
                            ...
                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('tender-delete')): ?>
                                &nbsp;
                                <button type="button" onclick="COnfirmDelete('<?php echo e($tender->id); ?>')"
                                    data-id="<?php echo e($tender->id); ?>" class="btn btn-sm btn-outline-danger">
    ...
    <script>
        $(document).ready(function() {
            var updateOutput = function(e) {
                ...
                $.ajax({
                    method: "POST",
                    url: "<?php echo e(route('admin.tender.order')); ?>",
                    data: {
        ...
        function COnfirmDelete(elem) {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        method: "POST",
                        url: '<?php echo e(route('admin.tender.destroy')); ?>',
                        data: {
...
<?php $__env->stopPush(); ?>
<?php /**PATH /home/megajobn/public_html/resources/views/admin/tender/components/list.blade.php ENDPATH**/ ?>