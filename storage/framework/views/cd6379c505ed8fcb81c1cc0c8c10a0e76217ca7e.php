    ...
    <div class="dd" id="nestable">
        <ol class="dd-list has-header">
            <?php $__currentLoopData = $joblevels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $JobLevel): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <li class="dd-item dd3-item list-item" data-id="<?php echo e($JobLevel->id); ?>">
                    <div class="dd-handle dd3-handle"></div>
                    <div class="custom-handle-flex">
                        <div class="icon-image-name">
                            <div>
                                <h6 class=" mb-0">
                                    <?php echo e($JobLevel->title); ?> | <small
                                        class="<?php echo e($JobLevel->status == 'active' ? 'badge badge-primary' : 'badge badge-danger'); ?>"><?php echo e($JobLevel->status == 'active' ? 'Displayed' : 'Not Display'); ?></small>
                                </h6>
                            </div>
                        </div>
                        <div style="display: flex">
                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('job-edit')): ?>
                                <a href="<?php echo e(route('admin.JobLevel.edit', base64_encode($JobLevel->id))); ?>"
                            ...
                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('job-delete')): ?>
                                &nbsp;
                                <button type="button" onclick="COnfirmDelete('<?php echo e($JobLevel->id); ?>')"
                                    data-id="<?php echo e($JobLevel->id); ?>" class="btn btn-sm btn-outline-danger">
    ...
    <script>
        $(document).ready(function() {
            var updateOutput = function(e) {
                ...
                $.ajax({
                    method: "POST",
                    url: "<?php echo e(route('admin.JobLevel.order')); ?>",
                    data: {
        ...
        function COnfirmDelete(elem) {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        method: "POST",
                        url: '<?php echo e(route('admin.JobLevel.destroy')); ?>',
                        data: {
                        ...
                        success: function(res) {
                            swal({
                                title: "Deleted",
                                text: "JobLevel has been deleted!",
                                type: "success",
                    ...
                    swal("Cancelled", "Your JobLevel is safe :)", "error");
                }
            });
        }
    </script>
<?php $__env->stopPush(); ?>
<?php /**PATH /home/megajobn/public_html/resources/views/admin/jobLevel/components/list.blade.php ENDPATH**/ ?>