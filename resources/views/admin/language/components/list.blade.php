    ...
    <div class="dd" id="nestable">
        <ol class="dd-list has-header">
            @foreach ($languages as $language)
                <li class="dd-item dd3-item list-item" data-id="{{ $language->id }}">
                    <div class="dd-handle dd3-handle"></div>
                    <div class="custom-handle-flex">
                        <div class="icon-image-name">
                            <div>
                                <h6 class=" mb-0">
                                    {{ $language->title }} | <small
                                        class="{{ $language->status == 'active' ? 'badge badge-primary' : 'badge badge-danger' }}">{{ $language->status == 'active' ? 'Displayed' : 'Not Display' }}</small>
                                </h6>
                            </div>
                        </div>
                        <div style="display: flex">
                            <a href="{{ route('admin.language.edit', base64_encode($language->id)) }}"
                                class="btn btn-sm btn-outline-info" title="Edit">
                                <i class="fa fa-edit"></i>
                            </a>
                            &nbsp;
                            <button type="button" onclick="COnfirmDelete('{{ $language->id }}')"
                                data-id="{{ $language->id }}" class="btn btn-sm btn-outline-danger">
    ...
    <script>
        $(document).ready(function() {
            var updateOutput = function(e) {
                ...
                $.ajax({
                    method: "POST",
                    url: "{{ route('admin.language.order') }}",
                    data: {
