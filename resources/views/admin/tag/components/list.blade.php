...
<div class="card">
    <div class="dd" id="nestable">
        <ol class="dd-list has-header">
            @foreach ($tags as $tag)
                <li class="dd-item dd3-item list-item" data-id="{{ $tag->id }}">
                    ...
                    <div class="custom-handle-flex">
                        <div class="icon-image-name">
                            <div>
                                <h6 class=" mb-0">
                                    {{ $tag->title }} | <small
                                        class="{{ $tag->status == 'active' ? 'badge badge-primary' : 'badge badge-danger' }}">{{ $tag->status == 'active' ? 'Displayed' : 'Not Display' }}</small>
                                </h6>
                            </div>
                        </div>
                        <div style="display: flex">
                            ...
                            @can('tender-delete')
                                ...
                                <button type="button" onclick="COnfirmDelete('{{ $tag->id }}')"
                                    data-id="{{ $tag->id }}" class="btn btn-sm btn-outline-danger">
