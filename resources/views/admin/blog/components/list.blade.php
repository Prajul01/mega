    ...
    <div class="dd" id="nestable">
        <ol class="dd-list has-header">
            @foreach ($blogs as $blog)
                <li class="dd-item dd3-item list-item" data-id="{{ $blog->id }}">
                    ...
                    <div class="custom-handle-flex">
                        <div class="icon-image-name">
                            <div>
                                <h6 class=" mb-0">
                                    {{ $blog->title }} | <small
                                        class="{{ $blog->status == 'active' ? 'badge badge-primary' : 'badge badge-danger' }}">{{ $blog->status == 'active' ? 'Displayed' : 'Not Display' }}</small>
                                </h6>
                            </div>
                        </div>
                        <div style="display: flex">
                            @can('content-edit')
                                <a href="{{ route('admin.blog.edit', base64_encode($blog->id)) }}"
                            ...
                            @can('content-delete')
                                ...
                                <button type="button" onclick="COnfirmDelete('{{ $blog->id }}')"
                                    data-id="{{ $blog->id }}" class="btn btn-sm btn-outline-danger">
