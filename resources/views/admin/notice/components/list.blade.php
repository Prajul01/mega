    ...
    <div class="dd" id="nestable">
        <ol class="dd-list has-header">
            @foreach ($notices as $notice)
                <li class="dd-item dd3-item list-item" data-id="{{ $notice->id }}">
                    ...
                    <div class="custom-handle-flex">
                        <div class="icon-image-name">
                            <div>
                                <h6 class=" mb-0">
                                    {{ $notice->title }} | <small
                                        class="{{ $notice->status == 'active' ? 'badge badge-primary' : 'badge badge-danger' }}">{{ $notice->status == 'active' ? 'Displayed' : 'Not Display' }}</small>
                                </h6>
                            </div>
                        </div>
                        <div style="display: flex">
                            <a href="{{ route('admin.notice.edit', base64_encode($notice->id)) }}"
                                class="btn btn-sm btn-outline-info" title="Edit">
                                <i class="fa fa-edit"></i>
                            </a>
                            &nbsp;
                            <button type="button" onclick="COnfirmDelete('{{ $notice->id }}')"
                                data-id="{{ $notice->id }}" class="btn btn-sm btn-outline-danger">
        ...
        function COnfirmDelete(elem) {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        method: "POST",
                        url: '{{ route('admin.notice.destroy') }}',
                        data: {
                            '_token': '{{ csrf_token() }}',
                            'id': id
                        },
