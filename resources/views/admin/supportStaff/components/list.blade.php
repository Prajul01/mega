    ...
    <div class="dd" id="nestable">
        <ol class="dd-list has-header">
            @forelse ($staffs as $staff)
                <li class="dd-item dd3-item list-item" data-id="{{ $staff->id }}">
                    <div class="dd-handle dd3-handle"></div>
                    <div class="custom-handle-flex">
                        <div class="icon-image-name">
                            <div>
                                <h6 class=" mb-0">
                                    <img src="{{ asset('/storage/supportStaff/' . $staff->profile_pic) }}"
                        ...
                        <div style="display: flex">
                            ...
                            <a href="{{ route('admin.staffs.edit', base64_encode($staff->id)) }}"
                            ...
                            &nbsp;
                            <button type="button" onclick="COnfirmDelete('{{ base64_encode($staff->id) }}')"
                                data-id="{{ $staff->id }}" class="btn btn-sm btn-outline-danger"><i
        ...
        function COnfirmDelete(elem) {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        method: "POST",
                        url: '{{ route('admin.staffs.destroy') }}',
                        data: {
                            '_token': '{{ csrf_token() }}',
                            'id': id
                        },
