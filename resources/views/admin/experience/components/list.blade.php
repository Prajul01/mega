    ...
    <div class="dd" id="nestable">
        <ol class="dd-list has-header">
            @foreach ($experiences as $experience)
                <li class="dd-item dd3-item list-item" data-id="{{ $experience->id }}">
                            ...
                            <div>
                                <h6 class=" mb-0">
                                    {{ $experience->title }} | <small
                                        class="{{ $experience->status == 'active' ? 'badge badge-primary' : 'badge badge-danger' }}">{{ $experience->status == 'active' ? 'Displayed' : 'Not Display' }}</small>
                                </h6>
                            </div>
                        </div>
                        <div style="display: flex">
                            @can('job-edit')
                                <a href="{{ route('admin.experience.edit', base64_encode($experience->id)) }}"
                            ...
                            @can('job-delete')
                                &nbsp;
                                <button type="button" onclick="COnfirmDelete('{{ $experience->id }}')"
                                    data-id="{{ $experience->id }}" class="btn btn-sm btn-outline-danger">
    ...
    <script>
        $(document).ready(function() {
            var updateOutput = function(e) {
                ...
                $.ajax({
                    method: "POST",
                    url: "{{ route('admin.experience.order') }}",
                    data: {
        ...
        function COnfirmDelete(elem) {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        method: "POST",
                        url: '{{ route('admin.experience.destroy') }}',
                        data: {
                            '_token': '{{ csrf_token() }}',
                            'id': id
                        },
