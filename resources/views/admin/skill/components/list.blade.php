    ...
    <div class="dd" id="nestable">
        <ol class="dd-list has-header">
            @foreach ($skills as $skill)
                <li class="dd-item dd3-item list-item" data-id="{{ $skill->id }}">
                    ...
                    <div class="custom-handle-flex">
                        <div class="icon-image-name">
                            <div>
                                <h6 class=" mb-0">
                                    {{ $skill->title }} | <small
                                        class="{{ $skill->status == 'active' ? 'badge badge-primary' : 'badge badge-danger' }}">{{ $skill->status == 'active' ? 'Displayed' : 'Not Display' }}</small>
                                </h6>
                            </div>
                        </div>
                        <div style="display: flex">
                            @can('job-edit')
                                <a href="{{ route('admin.skill.edit', base64_encode($skill->id)) }}"
                            ...
                            @can('job-delete')
                                &nbsp;
                                <button type="button" onclick="COnfirmDelete('{{ $skill->id }}')"
                                    data-id="{{ $skill->id }}" class="btn btn-sm btn-outline-danger">
