    ...
    <div class="dd" id="nestable">
        <ol class="dd-list has-header">
            @foreach ($districts as $district)
                <li class="dd-item dd3-item list-item" data-id="{{ $district->id }}">
                    ...
                    <div class="custom-handle-flex">
                        <div class="icon-image-name">
                            <div>
                                <h6 class=" mb-0">
                                    {{ $district->name }} | <small
                                        class="{{ $district->status == 'active' ? 'badge badge-primary' : 'badge badge-danger' }}">{{ $district->status == 'active' ? 'Displayed' : 'Not Display' }}</small>
                                </h6>
                            </div>
                        </div>
                        <div style="display: flex">
                            @can('location-edit')
                                <a href="{{ route('admin.district.edit', base64_encode($district->id)) }}"
                            ...
                            @can('location-delete')
                                ...
                                <button type="button" onclick="COnfirmDelete('{{ $district->id }}')"
                                    data-id="{{ $district->id }}" class="btn btn-sm btn-outline-danger">
    ...
    <script>
        $(document).ready(function() {
            var updateOutput = function(e) {
                ...
                $.ajax({
                    method: "POST",
                    url: "{{ route('admin.district.order') }}",
                    data: {
