...
<div class="card">
    <div class="dd" id="nestable">
        <ol class="dd-list has-header">
            @foreach ($faqs as $faq)
                <li class="dd-item dd3-item list-item" data-id="{{ $faq->id }}">
                    ...
                    <div class="custom-handle-flex">
                        <div class="icon-image-name">
                            <div>
                                <h6 class=" mb-0">
                                    {{ $faq->question }} | <small
                                        class="{{ $faq->display == 1 ? 'badge badge-primary' : 'badge badge-danger' }}">{{ $faq->display == 1 ? 'Displayed' : 'Not Display' }}
                                    </small>
                                </h6>
                            </div>
                        </div>
                        <div style="display: flex">
                            @can('content-edit')
                                <a href="{{ route('admin.companyFAQ.edit', base64_encode($faq->id)) }}"
                            ...
                            @can('content-delete')
                                ...
                                <button type="button" onclick="COnfirmDelete('{{ $faq->id }}')"
                                    data-id="{{ $faq->id }}" class="btn btn-sm btn-outline-danger">
        ...
        function COnfirmDelete(elem) {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        method: "POST",
                        url: '{{ route('admin.companyFAQ.destroy') }}',
                        data: {
                            '_token': '{{ csrf_token() }}',
                            'id': id
                        },
