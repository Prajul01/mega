    ...
    <div class="dd" id="nestable">
        <ol class="dd-list has-header">
            @foreach ($citys as $city)
                <li class="dd-item dd3-item list-item" data-id="{{ $city->id }}">
                    ...
                    <div class="custom-handle-flex">
                        <div class="icon-image-name">
                            <div>
                                <h6 class=" mb-0">
                                    {{ $city->name }} | <small
                                        class="{{ $city->status == 'active' ? 'badge badge-primary' : 'badge badge-danger' }}">{{ $city->status == 'active' ? 'Displayed' : 'Not Display' }}</small>
                                </h6>
                            </div>
                        </div>
                        <div style="display: flex">
                            @can('location-edit')
                                <a href="{{ route('admin.city.edit', base64_encode($city->id)) }}"
                            ...
                            @can('location-delete')
                                ...
                                <button type="button" onclick="COnfirmDelete('{{ $city->id }}')"
                                    data-id="{{ $city->id }}" class="btn btn-sm btn-outline-danger">
