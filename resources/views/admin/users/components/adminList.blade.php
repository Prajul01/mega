                ...
                @foreach ($users as $key => $user)
                    <tr>
                        ...
                        <td style="display:flex;">
                            <a href="{{ route('admin.admin-management.edit', base64_encode($user->id)) }}"
                                class="btn btn-outline-warning mr-1">
                                <i class="fa fa-edit"></i>
                            </a>
                            @can('user-edit')
                                @if (auth()->user()->id != $user->id)
                                    <form action="{{ route('admin.admin-management.suspend', base64_encode($user->id)) }}"
                            ...
                            @endcan
                            @can('user-delete')
                                <form action="{{ route('admin.admin-management.destroy', base64_encode($user->id)) }}"
