    ...
    <div class="dd" id="nestable">
        <ol class="dd-list has-header">
            @foreach ($industries as $industry)
                <li class="dd-item dd3-item list-item" data-id="{{ $industry->id }}">
                    ...
                    <div class="custom-handle-flex">
                        <div class="icon-image-name">
                            <div>
                                <h6 class=" mb-0">
                                    {{ $industry->name }} | <small
                                        class="{{ $industry->status == 'active' ? 'badge badge-primary' : 'badge badge-danger' }}">{{ $industry->status == 'active' ? 'Displayed' : 'Not Display' }}</small>
                                </h6>
                            </div>
                        </div>
                        <div style="display: flex">
                            @can('job-edit')
                                <a href="{{ route('admin.industry.edit', base64_encode($industry->id)) }}"
                            ...
                            @can('job-delete')
                                &nbsp;
                                <button type="button" onclick="COnfirmDelete('{{ $industry->id }}')"
                                    data-id="{{ $industry->id }}" class="btn btn-sm btn-outline-danger">
    ...
    <script>
        $(document).ready(function() {
            var updateOutput = function(e) {
                ...
                $.ajax({
                    method: "POST",
                    url: "{{ route('admin.industry.order') }}",
                    data: {
        ...
        function COnfirmDelete(elem) {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        method: "POST",
                        url: '{{ route('admin.industry.destroy') }}',
                        data: {
                            '_token': '{{ csrf_token() }}',
                            'id': id
                        },
