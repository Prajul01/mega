    ...
    <div class="dd" id="nestable">
        <ol class="dd-list has-header">
            @foreach ($careers as $career)
                <li class="dd-item dd3-item list-item" data-id="{{ $career->id }}">
                    ...
                    <div class="custom-handle-flex">
                        <div class="icon-image-name">
                            <div>
                                <h6 class=" mb-0">
                                    {{ $career->title }} | <small
                                        class="{{ $career->status == 'active' ? 'badge badge-primary' : 'badge badge-danger' }}">{{ $career->status == 'active' ? 'Displayed' : 'Not Display' }}</small>
                                </h6>
                            </div>
                        </div>
                        <div style="display: flex">
                            @can('career-edit')
                                <a href="{{ route('admin.career.edit', base64_encode($career->id)) }}"
                            ...
                            @can('career-delete')
                                ...
                                <button type="button" onclick="COnfirmDelete('{{ $career->id }}')"
                                    data-id="{{ $career->id }}" class="btn btn-sm btn-outline-danger">
